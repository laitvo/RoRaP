...
molecules = ['N2', 'O2-X', 'O2-a', 'O2-b']
radicals = ['NO-X', 'NO-a']
##non-ground (non-X) states may be optionally removed; it may be worth trying for O2 but is not recommendable for NO
...
def O_partition(T_e):
    '''Estimates an approximate partition sum to the three lowest O electronic states and calculates their ratios. Fraction relevant to the "rotational" Raman scattering, i.e., the microstates of the ground 3P term, are only returned.
    ...
    -------
    x_O2_3P2 : float
        ratio of the O(3P2) microstate (the ground state)
    x_O2_3P1 : float
        ratio of the O(3P1) microstate
    x_O2_3P0 : float
        ratio of the O(3P0) microstate
    '''

    q_O2 = 5. + 3.*np.exp(-h*c*100*158.265/(k*T_e)) + 1.*np.exp(-h*c*100*226.977/(k*T_e)) + 5.*np.exp(-h*c*100*15867.862 /(k*T_e)) + 1.*np.exp(-h*c*100*33792.583/(k*T_e))
    #wavenumbers and degeneracies taken from https://physics.nist.gov/cgi-bin/ASD/energy1.pl?de=0&spectrum=O+I&submit=Retrieve+Data&units=0&format=0&output=0&page_size=15&multiplet_ordered=0&conf_out=on&term_out=on&level_out=on&unc_out=1&j_out=on&lande_out=on&perc_out=on&biblio=on&temp=

    x_O2_3P2 = 5./q_O2
    x_O2_3P1 = 3.*np.exp(-h*c*100*158.265/(k*T_e))/q_O2
    x_O2_3P0 = 1.*np.exp(-h*c*100*226.977/(k*T_e))/q_O2

    return x_O2_3P2, x_O2_3P1, x_O2_3P0


def NO_partition(T_e):
    '''Estimates an approximate partition sum to the two NO sublevels and calculates their ratios.
    ...
    -------
    x_NO_X : float
        ratio of the NO (J=1/2) ground state
    x_NO_a : float
        ratio of the NO (J=3/2)
    '''

    q_NO = 1. + 4.*np.exp(-h*c*100.*119.73/(k*T_e))
    ##wavenumbers taken from https://webbook.nist.gov/cgi/cbook.cgi?ID=C10102439&Mask=1000
 
    x_NO_X = 1./q_O2
    x_NO_a = 4.*np.exp(-h*c*100.*119.73/(k*T_e))/q_O2

    return x_NO_X, x_NO_a
...
def plasma(params, wn_0, spectrum):
    ...
    x_N2 = params['x_N2'].value
    x_NO = params['x_NO'].value
    ...
    x_NO_X, x_NO_a = x_NO*np.array(NO_partition(T_e))
    ##to be commented out if either metastables are not to be detected

    abundances = {'N2': x_N2, 'O2-X':x_O2_X, 'O2-a': x_O2_a, 'O2-b': x_O2_b, 'NO-X': x_NO_X, 'NO-a': x_NO_a}
    ##any metastables may be removed up to a minimalistic example replacing the above with`abundances = {'CO2': x_CO2, 'CO': x_CO, 'O2-X': x_O2}'
    ...
    for species in radicals:

        delta_J_radicals = [1.,2.]

        for delta_J in delta_J_radicals:

            E_J, E_delta_J = raman.E_J(J+0.5, raman.B0[species], raman.D0[species], delta_J)
            nu_J = raman.nu_J(E_J, E_delta_J)
    ...
    x_O2_3P2, x_O2_3P1, x_O2_3P0 = x_O*np.array(O_partition(T_e))
    I_O = dSigma_dOhm_O*x_O2_3P2

    signal_O = raman.roRaman_spectrum(wn, nu_O, I_O, np.ones(len(nu_O))*fwhm)[1]
    I_wn += signal_O
...
wn_off = 0.
##declaring O atom cross sections from literature
dSigma_dOhm_O = np.array([5.27e-031, 2.11e-031])*1e031 #given in the order of [2-1, 2-0] transitions https://www.osti.gov/pages/servlets/purl/1877143
...
param.add('x_N2', value=x_N2, vary=True, min=0., max=1.)
param.add('x_O2', value=x_O2, vary=True, min=0.1, max=1.)
param.add('x_NO', value=x_NO, vary=True, min=0.1, max=1.)
param.add('x_O', value=x_O, vary=True, min=0.1, max=1.)
